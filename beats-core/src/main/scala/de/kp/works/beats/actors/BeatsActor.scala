package de.kp.works.beats.actors
...
import de.kp.works.beats.{BeatsConf, BeatsLogging, Response}

import scala.concurrent.duration._
import scala.concurrent.{Await, ExecutionContextExecutor}
import scala.util.Try

abstract class BeatsActor(name:String, queue: SourceQueueWithComplete[String]) extends Actor with BeatsLogging {
  ...
  val config: Config = BeatsConf.getBeatCfg(name)
