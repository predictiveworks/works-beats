...
import de.kp.works.beats.tls.redis.{OsqueryNode, RedisApi}
...
abstract class BaseActor extends Actor with ActorLogging {
  ...
  val osqueryCfg: Config = BeatsConf.getBeatCfg(BeatsConf.OSQUERY_CONF)
  ...
  def getNode(request:HttpRequest):OsqueryNode = {
    ...
    val nodeKey = try {
      payload.get(NODE_KEY).getAsString
    ...
    var node = RedisApi.nodeByKey(nodeKey)
    /*
