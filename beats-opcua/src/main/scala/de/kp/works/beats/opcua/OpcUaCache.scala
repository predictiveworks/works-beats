...
class OpcUaCache(client:OpcUaClient) {
  ...
  private val maximumSize = addressCacheCfg.getInt("maximumSize")
    ...
    def browseInline(startNodeId: NodeId, maxLevel: Int, level: Int, flat: Boolean, path: String=""): JsonArray = {
      ...
      try {
        ...
        if (browseResult.getStatusCode.isGood && browseResult.getReferences != null) {
          ...
          LOGGER.error(s"Browsing nodeId [$startNodeId] failed [${browseResult.getStatusCode.toString}]")
        }

      } catch {
        case e: InterruptedException =>
          LOGGER.error(s"Browsing nodeId [$startNodeId] exception: [${e.getLocalizedMessage}]")
