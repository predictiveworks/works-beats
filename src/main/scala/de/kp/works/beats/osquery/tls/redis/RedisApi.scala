...
object RedisApi {
  ...
  */
  ...
  private val redisDB = RedisDB.getInstance
  /**
  ...
  def nodeConfig(uuid:String):JsonObject = {
    ...
    val config:String = redisDB.readConfigByNode(uuid)
  ...
  def nodeBySecret(secret:String):OsqueryNode =
    redisDB.readNodeBySecret(secret)
  /**
   ...
   */
  def taskById(uuid:String):OsqueryQueryTask = redisDB.readTaskById(uuid)
