package de.kp.works.beats.osquery.schema
/*
...
object Schema {
  ...
  private var types:Option[List[String]] = None
  
  private val createTable = "CREATE TABLE ([a-z0-9_]+) \\(([a-zA-z0-9, ]+)\\);".r
  private val alterTable = "ALTER TABLE ([a-z0-9_]+) ADD ([a-zA-z0-9, ]+);".r

  /* 
   * Build tables and names as in-memory data;
   * this is the prerequisite to transform query
   * results into NGSI-v2 format 
   */
  build()
  ...
  def getColumnMap(name:String):Map[String,String] = {

    if (tables.isEmpty) return Map.empty[String,String]
    if (isTable(name)) {
