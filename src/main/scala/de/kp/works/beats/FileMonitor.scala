...
abstract class FileMonitor(name:String, folder:String) {
  ...
  private val beatsCfg = BeatsConf.getBeatCfg(name)
  private val receiverCfg = beatsCfg.getConfig("receiver")
   ...
   * use cases somewhat.
   */
  private val systemName:String = receiverCfg.getString("systemName")
